  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const [metricsRes, queueRes] = await Promise.all([
        ...
        axios.get('http://localhost:8000/api/queue/metrics', {
          headers: { Authorization: `Bearer ${token}` }
        })
      ]);
      ...
      setMetrics({
        ...
        completionRate: parseFloat(metricsRes.data.completion_rate.replace('%', '')),
          ...
          <Paper sx={{ p: 3 }}>
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
              <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <Chip 
                  label={metrics.pendingTests + metrics.inProgressTests > 0 ? 'Active' : 'Empty'} 
                  color={metrics.pendingTests + metrics.inProgressTests > 0 ? 'warning' : 'success'}
                  size="small"
                />
              </Box>
              <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
